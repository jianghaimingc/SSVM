...
Expect<Plugin> Plugin::load(const std::filesystem::path &Path) noexcept {
  ...
  if (auto Func = P.Lib->get<decltype(ssvmGetPluginInfo)>("ssvmGetPluginInfo");
      unlikely(!Func)) {
  ...
  if (P.Info.APIVersion != SSVM_PLUGIN_API_VERSION) {
    return P;
  }
...
Plugin::enumerate(const std::filesystem::path &SearchPath) {
...
#if defined(SSVM_OS_LINUX) || defined(SSVM_OS_MACOS)
  ...
  auto LocalPluginDir = Home / ".ssvm"sv / "plugins"sv;
